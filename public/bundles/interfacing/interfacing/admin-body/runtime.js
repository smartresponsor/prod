...
const HYDRATING_ATTR = 'adminBodyHydrating';
const RETRY_CONTROLLER_ATTR = '__interfacingAdminBodyRetryController';
const RETRY_EVENTS = [
...
function hydrateMount(mount) {
  if (mount.dataset[HYDRATION_ATTR] === 'ready' || mount.dataset[HYDRATING_ATTR] === 'true') {
    return;
  }

  mount.dataset[HYDRATING_ATTR] = 'true';

  const schema = parseSchema(mount);

  if (!schema) {
    mount.dataset[HYDRATING_ATTR] = 'false';
    return;
  }
  ...
  if (provider && typeof provider.mount === 'function') {
    try {
      ...
      mount.dataset[HYDRATING_ATTR] = 'false';
      if (mount[RETRY_CONTROLLER_ATTR]) {
        mount[RETRY_CONTROLLER_ATTR].abort();
        mount[RETRY_CONTROLLER_ATTR] = null;
      }
      ...
      mount.dataset[HYDRATION_ATTR] = 'provider-mount-error';
      mount.dataset[HYDRATING_ATTR] = 'false';
      if (mount[RETRY_CONTROLLER_ATTR]) {
        mount[RETRY_CONTROLLER_ATTR].abort();
        mount[RETRY_CONTROLLER_ATTR] = null;
      ...
      mount.dispatchEvent(new CustomEvent(HYDRATION_FAILED_EVENT, {
        bubbles: true,
        detail: {
          reason: 'provider-mount-error',
          error,
          resource: schema?.resource?.name || schema?.resource,
          operation: schema?.operation,
          surface: schema?.surface,
          provider: providerName,
          hydration: 'provider-mount-error',
        },
      }));
  ...
  mount[RETRY_CONTROLLER_ATTR] = retryController;

  const retry = () => {
    if (retryController.signal.aborted || mount.dataset[HYDRATION_ATTR] === 'ready') {
      return;
    }
  ...
  RETRY_EVENTS.forEach((eventName) => {
    window.addEventListener(eventName, retry, { signal: retryController.signal });
  });
